...
import com.tencent.service.PayQueryService;
...
public class ScanPayBusiness {
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            log.e("return_msg: "+scanPayResData.getReturn_msg());
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID") || errorCode.equals("NOTENOUGH")) {
                    ...
                    //对于扣款明确失败的情况直接走撤销逻辑,这里的撤销只可能用商户订单号标识
                    ...
                    //表示有可能单次消费超过1000元，或是免输密码消费次数已经超过当天的最大限制(5次)，或者符合微信后台的风控规则
                	//这个时候提示用户输入密码，商户自己隔一段时间去查单，查询一定次数，看用户是否已经输入了密码
                	//此时只有商户订单号，没有微信订单号
    ...
    /**
     * 进行一次支付订单查询操作,注意这里商户可以改造成异步线程查询,不然对于大量订单会造成延迟
     *
    ...
    private boolean doOnePayQuery(String outTradeNo) throws Exception {
        ...
        PayQueryReqData scanPayQueryReqData = new PayQueryReqData("",outTradeNo);
        payQueryServiceResponseString = payQueryService.request(scanPayQueryReqData);
    ...
    /**
     * 由于有的时候是因为服务延时，所以需要商户每隔一段时间（建议5秒）后再进行查询操作，多试几次（建议6次）
     *
    ...
    private boolean doOneReverse(String outTradeNo) throws Exception {
        ...
        if (reverseResData.getReturn_code().equals("FAIL")) {
            ...
            if (reverseResData.getResult_code().equals("FAIL")) {
                ...
                //查询成功，打印交易状态, 这里商户测可以自己增加一些数据存储的逻辑
                log.i("支付订单撤销成功");
                return true;
            }
        }
    ...
    public void setScanPayQueryService(PayQueryService service) {
        payQueryService = service;
    }
